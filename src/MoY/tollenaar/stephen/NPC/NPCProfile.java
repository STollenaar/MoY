...
public class NPCProfile {
	...
	public static NPCProfile loadUrl(String name, String owner) {
		GameProfile prof = new GameProfile(UUID.randomUUID(), name);
		String value = ProfileFactory(owner, prof.getId(), name);
		prof.getProperties().put("textures", new Property(value, name));
		return new NPCProfile(prof, name);
	}
		...
		if (TEXTURE_CACHE.get(skinuuid.toString()) != null) {
			...
			if (SKIN_THREAD == null) {
				Bukkit.getScheduler().runTaskTimer(
						...
						SKIN_THREAD = new SkinThread(), 10, 10);
			}
			try {
			...
			UUID realuuid = Bukkit.getOfflinePlayer(real).getUniqueId();
			if (TEXTURE_CACHE.get(realuuid.toString()) != null) {
				return TEXTURE_CACHE.get(realuuid.toString());
			} else {
				return TEXTURE_CACHE.get(skinuuid.toString());
			}
		}
	...
	private static String ProfileFactory(String url, UUID uuid, String name) {

		String timestamp = "{\"timestamp\":"
				+ Long.toString(System.currentTimeMillis());
		String profileid = ",\"profileId\":\""
				+ uuid.toString().replace("-", "") + "\"";
		String profilename = ",\"profileName\":\"" + name + "\"";
		String texture = ",\"textures\":{\"SKIN\":{\"url\":\"" + returnUrl(url)
				+ "\"}}}";

		String value = Base64Coder.encodeString(timestamp + profileid
				+ profilename + texture);
		return value;
	}
