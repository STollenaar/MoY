...
public class Filewriters {
	...
	public void defaultymlvalues(FileConfiguration config){
		config.options().header("All parts with a %escapethingy% has to be in that sentence. Don't add more of these and don't change the sign of these. You can place them in a different order.");
		config.set("DragonNotEnoughMoney",  "Sorry you don't have enough money to board this dragon coach.");
		...
		config.set("AlmostLeaving", "The %type% is about to leave. Hurry you only have %time% left.");
		config.set("Boarding", "You board the %type%. And a friendly crewmember lets you know that it leaves in %time% %min% and %sec% seconds.");
		
		config.set("QuestComplete", "You have completed the quest %questname%. Go back to %npc% to claim your reward.");
		config.set("HarvestProg", "The quest %name% has made progress. You have mined %count% of the block %block%.");
		config.set("KillProg", "The quest %name% has made progress. You have killed %count%  %monster%'s.");
		...
		config.set("Partylvlup", "You have gained 1 level thanks to %playername%. You are now level %lvl%.");
		config.set("Partyname.tolong", "This partyname is too long use less than 50 characters.");
		config.set("Partyname.taken", "This partyname is already taken.");
		config.set("Partyjoin", "%member% has joined the party.");
		config.set("Partyinvite.victim", "%leader% invited you for the party %partyname%.");
		config.set("Partyinvite.leader", "You invited %player% for the party %partyname%.");
		...
		config.set("Partykick.himself", "One simply can't ick themself out of the party.");
		config.set("newpartyleader.wrongparty", "New party leader is not in your party.");
		config.set("newpartyleader.notinparty", "New party leader is not in a party.");
		...
		config.set("Partyleave.leader", "You can't leave the party as a party leader. Disband the party or assign a new leader.");
	}
