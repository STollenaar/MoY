...
public class SpeechCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label,
		...
		if (args.length == 6 && sender instanceof Player
			...
			String npcname = plugin.getNPCHandler().getNPCByUUID(npcuuid)
					.getName();
			int questType = Integer.parseInt(args[4]);
			int currentNode =  Integer.parseInt(args[5]);
			if (isCache(player.getUniqueId(), currentNode)) {
				if (speechType == SpeechType.ACCEPT) {
					if (questType != -5) {
						QuestProcessEvent event = new QuestProcessEvent(player,
								node, getQuestType(questType));
	...
	public boolean isCache(UUID player, int number) {
